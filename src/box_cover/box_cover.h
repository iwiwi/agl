...
class coverage_manager {
 ...
 public:
  coverage_manager(const agl::G &g, const agl::W r, const double c) : cnt(0), radius(r), least_coverage(c) {
...
void select_greedily(const agl::G &g, const std::vector<std::vector<agl::V>> &X, std::vector<agl::V> &centers, std::vector<bool> &centered, const int k, coverage_manager &cm);
