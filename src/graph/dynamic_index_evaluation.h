...
struct dynamic_update_scenario {
...
//
template<typename GraphType = G>
class dynamic_update_scenario<GraphType>::update_interface {
 public:
  virtual void apply(GraphType *g, graph_dynamic_index_interface<GraphType> *i) const = 0;
};

template<typename GraphType = G>
class dynamic_update_scenario<GraphType>::update_add_edge
: public dynamic_update_scenario<GraphType>::update_interface {
 public:
...
template<typename GraphType = G>
class dynamic_update_scenario<GraphType>::update_remove_edge
: public dynamic_update_scenario<GraphType>::update_interface {
 public:
  V v_from, v_to;
...
//
template<typename GraphType = G>
class dynamic_update_scenario<GraphType>::generator {
 public:
  using edge_list_type = typename GraphType::edge_list_type;
  using scenario_type = dynamic_update_scenario<GraphType>;

  static void add_workload_edge_addition(scenario_type *s, const edge_list_type &update_es) {
    assert(s != nullptr);
    s->workloads.emplace_back();
    auto &w = s->workloads.back();
  ...
  static void add_workload_edge_addition_and_removal(scenario_type *s, const edge_list_type &update_es) {
    assert(s != nullptr);
    add_workload_edge_addition(s, update_es);
  ...
  static void add_workload_edge_addition_and_removal_random(scenario_type *s, size_t num_update_es) {
    assert(s != nullptr);

    edge_list_type update_es(num_update_es);
    std::unordered_set<std::pair<V, V>> update_es_set;

    std::uniform_int_distribution<V> rng(0, s->initial_graph.num_vertices() - 1);
    for (size_t i = 0; i < num_update_es; ++i) {
      V u, v;
      do {
        u = rng(agl::random);
        v = rng(agl::random);
      } while (u == v || is_adjacent(s->initial_graph, u, v) || update_es_set.count({u, v}));

      update_es_set.insert({u, v});
      update_es[i] = {u, v};
    }

    add_workload_edge_addition_and_removal(s, update_es);
  }
};
...
//
template<typename GraphType = G>
class dynamic_update_scenario<GraphType>::evaluator {
 public:
  using scenario_type = dynamic_update_scenario<GraphType>;
  ...
  static scenario_result evaluate(const scenario_type &s, dynamic_index_type *i) {
    scenario_result r;
    {
      // Construction
      double t = get_current_time_sec();
      i->construct(s.initial_graph);
    ...
    {
      // Update workloads
      r.workload_results.resize(s.workloads.size());
      nothing_index ni;
      std::vector<double> t0 = evaluate_internal(s, &ni);
      std::vector<double> t1 = evaluate_internal(s, i);
      for (size_t i : make_irange(s.workloads.size())) {
        ...
        wr.average_index_update_time_sec = (t1[i] - t0[i]) / s.workloads[i].size();
 ...
 private:
  class nothing_index : public dynamic_index_type {
   public:
  ...
  static std::vector<double> evaluate_internal(const scenario_type &s, dynamic_index_type *i) {
    std::vector<double> r;
    GraphType g = s.initial_graph;
...
void pretty_print(const dynamic_update_scenario<GraphType> &s, std::ostream &os = std::cerr) {
