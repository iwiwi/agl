...
def MEMB(G, rb, cycle=0):
    ...
    # Dict with {central_distance:[nodes]}
    nodes_sorted_by_central_distance = {}
    # This contains [(node:excluded_mass)] for rb
    excluded_mass_of_non_centers_rb = {}
    # This contains [(node:excluded_mass)] for rb+1
    ...
    for node in non_center_nodes:
        ...
        try:
            excluded_mass_of_non_centers_rb2[
                excluded_mass_of_node].append(node)
        except KeyError:
        ...
        while 1:
            if rb2 == rb + 1:
                ...
                while 1:
                    ...
                    node = random.choice(
                        excluded_mass_of_non_centers_rb2[maximum_key])
                    if node in center_nodes:
                        excluded_mass_of_non_centers_rb2[
                            maximum_key].remove(node)
                        if not excluded_mass_of_non_centers_rb2[maximum_key]:
                            del excluded_mass_of_non_centers_rb2[maximum_key]
                    else:
                        break
                nodes_visited = {}
                ...
                excluded_mass_of_node = len(
                    set(nodes_visited.keys()).difference(covered_nodes))
                ...
                while 1:
                    ...
                    node = random.choice(
                        excluded_mass_of_non_centers_rb[maximum_key])
                    if node in center_nodes:
                        excluded_mass_of_non_centers_rb[
                            maximum_key].remove(node)
        ...
        for i in new_covered_nodes:
            ...
            try:
                if central_distance_of_node[i] > new_covered_nodes[i]:
                    nodes_sorted_by_central_distance[
                        central_distance_of_node[i]].remove(i)
                    if not nodes_sorted_by_central_distance[central_distance_of_node[i]]:
                        del nodes_sorted_by_central_distance[
                            central_distance_of_node[i]]
                    try:
                        nodes_sorted_by_central_distance[
                            new_covered_nodes[i]].append(i)
                    except KeyError:
                        nodes_sorted_by_central_distance[
                            new_covered_nodes[i]] = [i]
    ...
    for i in range(1, max_distance + 1):
        for j in nodes_sorted_by_central_distance[i]:
            targets = list(set(adj[j].iterkeys()).intersection(
                set(nodes_sorted_by_central_distance[i - 1])))
