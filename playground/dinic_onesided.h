...
class dinic_onesided {
  ...
  bool one_sided_bfs(int s, int t) {
    queue<int> qs, qt;
    qs.push(s); qt.push(t);
...
public:
  ...
  dinic_onesided(const vector<pair<V, V>>& edges, int num_vs)
  ...
  int max_flow(int s, int t) {
    ...
    for (; ; s_side_bfs_revision += 2, t_side_bfs_revision += 2) {
      bool path_found = one_sided_bfs(s, t);
