...
class PrunedLandmarkLabeling {
 public:
  ...
  PrunedLandmarkLabeling()
      : num_v_(0), index_(NULL), index_l_(0), time_load_(0), time_indexing_(0) {}
...
template<int kNumBitParallelRoots>
const uint32_t PrunedLandmarkLabeling<kNumBitParallelRoots>::INF32 = numeric_limits<int32_t>::max();  // signed for safety
template<int kNumBitParallelRoots>
...
::ConstructIndex(const std::vector<std::pair<int, int> > &es) {
  ...
  max_threads = get_max_threads();
  {
    ...
    std::vector<std::pair<std::vector<int>, std::vector<uint8_t> > >
        tmp_idx(V, make_pair(std::vector<int>(1, INF32),
...
::PartialBPBFS(int r, int u, int v) {
  ...
  for (int d = base_d; h0 > 0; ++d) {
    ...
    rep (i0, h0) queued[que0[i0]] = false;
    que0.swap(que1);
    h0 = h1;
    que1.clear();
    h1 = 0;
...
int main(int argc, char **argv) {
  ...
  //
  {
    for (;;) {
      ...
      cout << pll.QueryDistance(from, to) << endl;
    }
  }
