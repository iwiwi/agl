...
class PrunedLandmarkLabeling {
 public:
  ...
  inline int QueryDistance(int from, int to);
...
::ConstructIndex(const std::vector<std::pair<int, int> > &es) {
  ...
  std::vector<std::vector<int>> relabelled_adj[2];
  {
    // Order
    ...
    relabelled_adj[0].resize(V), relabelled_adj[1].resize(V);
    std::vector<int> rank(V);
    for (int i = 0; i < V; ++i) rank[deg[i].second] = i;
    for (int from = 0; from < V; ++from) {
      for (size_t i = 0; i < adj_[0][from].size(); ++i) {
        int to = adj_[0][from][i];
        relabelled_adj[0][rank[from]].push_back(rank[to]);
        relabelled_adj[1][rank[to]].push_back(rank[from]);
      }
    }
  }
  ...
  for (int x = 0; x < 2; ++x) {
    ...
    for (int r = 0; r < V; ++r) {
      ...
      for (int dist = 0; que_t0 < que_h; ++dist) {
        for (int que_i = que_t0; que_i < que_t1; ++que_i) {
          int v = que[que_i];
          std::pair<std::vector<int>, std::vector<uint8_t>> &tmp_idx_v =
              tmp_idx[v];
          assert(v < (int)inv.size());
          index_t &idx_v = idx[inv[v]];
          ...
          for (size_t i = 0; i < tmp_idx_v.first.size() - 1; ++i) {
            ...
            int tmp_dist = tmp_idx_v.second[i] + dst_r[w];
...
template <int kNumBitParallelRoots>
int PrunedLandmarkLabeling<kNumBitParallelRoots>::QueryDistance(int from, int to) {
  if (from == to) return 0;
  if (from >= num_v_ || to >= num_v_) return INT_MAX;

  const index_t &idx_from = idx_[0][from];
  const index_t &idx_to = idx_[1][to];
  int dist = INF8;
  ...
  // }
  for (int i1 = 0, i2 = 0;;) {
    uint32_t v1 = idx_from.spt_v[i1], v2 = idx_to.spt_v[i2];
    if (v1 == v2) {
      if (v1 == INF32) break;  // Sentinel
      // printf(" %d->%d->%d: %d+%d\n", v, ord_[v1], w, idx_v.spt_d[i1], idx_w.spt_d[i2]);
      int tmp_dist = idx_from.spt_d[i1] + idx_to.spt_d[i2];
