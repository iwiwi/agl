...
class PrunedLandmarkLabeling {
 public:
  ...
  bool ConstructIndexDirected(const std::vector<std::pair<int, int> > &es);
  ...
  std::vector<std::vector<int>> directed_adj_[2];
  index_t *index_;
...
template <int kNumBitParallelRoots>
bool PrunedLandmarkLabeling<kNumBitParallelRoots>
::ConstructIndexDirected(const std::vector<std::pair<int, int> > &es) {
  //
  // Prepare the adjacency list and index space
  //
  Free();
  ...
  directed_adj_[0].resize(V);
  directed_adj_[1].resize(V);
  for (size_t i = 0; i < es.size(); ++i) {
    int from = es[i].first, to = es[i].second;
    directed_adj_[0][from].push_back(to);
    directed_adj_[1][to].push_back(from);
  }

  // initialize index
  index_ = (index_t *)memalign(64, 2 * V * sizeof(index_t));
  index_l_ = V;
  if (index_ == NULL) {
